...
## Solution (Buy or Wait? agent)

Deterministic LLM+Python pipeline; every output number traces to pure Python
(the model layer is optional and only ever phrased explanation text).
...
Flow per request: `FinancialState` (linked-event chain resolution) ->
message/image interpreters (multilingual, injection-immune, OCR for blank
amounts) -> currency normalization (directional dated FX graph) ->
candidate plans (full / partial / installments / wait / spending changes,
each with an eligibility trail) -> 90-day deterministic forecast ->
